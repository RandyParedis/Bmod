...
class BmodGlobalScopeProvider extends ImportUriGlobalScopeProvider {
	...
	override protected getImportedUris(Resource resource) {
		return cache.get(BmodGlobalScopeProvider.getSimpleName(), resource, new Provider<LinkedHashSet<URI>>() {
			override get() {
			...
			def LinkedHashSet<URI> collectImportUris(Resource resource, LinkedHashSet<URI> uniqueImportURIs) {
				...
				val models = resourceDescription.getExportedObjectsByType(BmodPackage.Literals.FLOORPLAN)
				println(models)
				
				models.forEach[
					val userData = getUserData(BmodResourceDescriptionStrategy.INCLUDES)
					if(userData !== null) {
